...
class LI02_양도소득_과세표준확정신고_기본정보(ERSRecord):
    ...
    양도일자: str = field(default=None, metadata={'max_length': 8, '점검': '', '비고': 'Not null (2)'})
...
class 양도소득_개인지방소득세신고(ERSReport):
    ...
    def __init__(self, 국세신고: 양도소득세신고):
        super().__init__()
        self.LI01_양도소득_과세표준확정신고_HEADER = [LI01_양도소득_과세표준확정신고_HEADER(
            세무대리인_사업자등록번호=국세신고.세무대리인.사업자등록번호,
        ...
        self.LI03_양도소득_과세표준확정신고_세율별내역 = [
            LI03_양도소득_과세표준확정신고_세율별내역(
                자료구분2='1' if record.세율구분 == '00' else '2',
                ...
                전자신고_세액공제=0,
            ) for record in sorted(국세신고.TI03_양도소득과세표준신고서_세율별내역, key=lambda r: r.세율구분) if record.국내외분 != 'Z'
        ]
